...
from TCP_Protocol_WithoutCongestion import rdt_Sender, rdt_Receiver
...
channel_for_data  = UnreliableChannel(env=env,name="DATA_CHANNEL",Pc=0,Pl=0.02,propagation_delay=2, transmission_rate=1000, bandwidth = 10)
channel_for_ack	  = UnreliableChannel(env=env,name="ACK_CHANNEL", Pc=0,Pl=0.02,propagation_delay=2, transmission_rate=1000, bandwidth = 10)
...
TOTAL_SIMULATION_TIME=2000  # <==== Total simulation time. Increase it as you like.
t=0
while True:
	if(env.peek()>t):
